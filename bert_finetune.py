...
def load_eng_words():
    ...
    with open('/home/ayushagarwal/deepjump/words_alpha.txt') as word_file:
...
def load_labels():
    ...
    labels = pd.read_csv('/home/ayushagarwal/deepjump/jumps_by_day.csv')
    ...
    labels['Date'] = pd.to_datetime(
        labels['Date'], errors='coerce', infer_datetime_format=True)
...
def load_articles(narts=5, nwords=100):
    ...
    ind = 0
    for i, art in enumerate(os.listdir('/home/ayushagarwal/deepjump/WSJ_txt')):
        # print(art)
        if i > narts:
            break
